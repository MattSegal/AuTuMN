...
def calc_compartment_periods(periods: Dict[str, float], periods_calculated: dict) -> Dict[str, float]:
    """
    Calculate compartment periods from provided parameters.
    """
    final_periods = {**compartment_periods}
    for calc_period_name, calc_period_def in periods_calculated.items():
        ...
        total_props = 0
        for comp_name, prop in calc_period_def.items()
